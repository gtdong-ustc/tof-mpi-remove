...
def get_metrics_mae(depth, ori_depth, gt, msk):
    ...
    if msk == None:
        ...
        # gt_list = tf.split(gt, batch_size, axis=0)
        # depth_list = tf.split(depth, batch_size, axis=0)
        # ori_depth_list = tf.split(ori_depth, batch_size, axis=0)
        # msk_list = tf.split(msk, batch_size, axis=0)
        # for i in range(batch_size):
        #     msk_mean = tf.reduce_mean(msk_list[i])
        #     gt_list[i] = gt_list[i] * msk_list[i] / msk_mean
        #     depth_list[i] = depth_list[i] * msk_list[i] / msk_mean
        #     ori_depth_list[i] = ori_depth_list[i] * msk_list[i] / msk_mean
        # gt = tf.concat(gt_list, axis=0)
        # depth = tf.concat(depth_list, axis=0)
        # ori_depth = tf.concat(ori_depth_list, axis=0)
